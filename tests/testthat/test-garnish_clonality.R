testthat::test_that("garnish_clonality", {
  ...
  testthat::expect_equal(dto[!is.na(cl_proportion), cl_proportion %>% range(na.rm = TRUE)], c(0.2720, 1.0484))
...
testthat::expect_equal(dt[!is.na(cl_proportion), cl_proportion %>% range(na.rm = TRUE)] %>% signif(digits = 3), c(0.259, 1.000))
