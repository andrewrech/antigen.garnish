...
#' * **fs_neos**: mutant nmers derived from frameshift variants predicted to bind MHC with at least some affinity (> 1000nM \eqn{IC_{50}})
#' * **fusion_neos**: mutant nmers derived from fusion variants predicted to bind MHC with at least some affinity (> 1000nM \eqn{IC_{50}})
...
# summarize over unique nmers
  ...
  ##need a value in DAI for next step, so if NA make it zero, suppress warning from data.table package for column class change.
  ##this ccures with jaffa, direct peptide, or direct cDNA input.
  suppressWarnings(
  dt[is.na(DAI), DAI := 0]
            )
  ...
  Class_i <- "(HLA-[ABC]\\*)|(H-2-[A-Z][a-z])"
  
  Class_ii <- "(HLA-D[A-Z0-9]+\\*)|(H-2-[A-Z]{2}[a-z])"
  ...
  dtn <- parallel::mclapply(dt[, sample_id %>% unique], function(id){
    ...
    if (!("effect_type" %chin% names(dt)) %>% any) dt[, effect_type := NA]
      ...
      return(
          ...
          sample_id = id,
          priority_neos_class_I = dt[MHC %like% Class_i &
                                        DAI > 10 & 
                                         Consensus_scores < 50] %>% nrow,
          priority_neos_class_II = dt[MHC %like% Class_ii &
                                        DAI > 10 & 
                                        Consensus_scores < 50] %>% nrow,
          classic_neos_class_I = dt[MHC %like% Class_i & pep_type == "mutnfs" & Consensus_scores < 50] %>%
                                                                              data.table::as.data.table %>%  nrow,
          classic_neos_class_II = dt[MHC %like% Class_ii & pep_type == "mutnfs" & Consensus_scores < 50] %>%
                                                                              data.table::as.data.table %>%  nrow,
          classic_top_score_class_I = dt[MHC %like% Class_i & pep_type == "mutnfs" & Consensus_scores < 5000, (1/Consensus_scores) %>% sum_top_v],
          classic_top_score_class_II = dt[MHC %like% Class_ii & pep_type == "mutnfs" & Consensus_scores < 5000, (1/Consensus_scores) %>% sum_top_v],
          alt_neos_class_I = dt[MHC %like% Class_i & pep_type == "mutnfs" & Consensus_scores < 5000 & DAI > 10] %>%
                                                                                          data.table::as.data.table %>% nrow,
          alt_neos_class_II = dt[MHC %like% Class_ii & pep_type == "mutnfs" & Consensus_scores < 5000 & DAI > 10] %>%
                                                                                          data.table::as.data.table %>%  nrow,
          alt_top_score_class_I = dt[MHC %like% Class_i & pep_type == "mutnfs" & Consensus_scores < 5000, DAI %>% sum_top_v],
          alt_top_score_class_II = dt[MHC %like% Class_ii & pep_type == "mutnfs" & Consensus_scores < 5000, DAI %>% sum_top_v],
          fs_neos_class_I = dt[MHC %like% Class_i & effect_type %like% "frameshift" & Consensus_scores < 1000] %>% 
                                                                                          data.table::as.data.table %>% nrow,
          fs_neos_class_II = dt[MHC %like% Class_ii & effect_type %like% "frameshift" & Consensus_scores < 1000]  %>% 
                                                                                          data.table::as.data.table %>% nrow,
          fusion_neos_class_I = dt[MHC %like% Class_i & !is.na(fusion_uuid) & Consensus_scores < 1000] %>% 
                                                                                        data.table::as.data.table %>% nrow,
          fusion_neos_class_II = dt[MHC %like% Class_ii & !is.na(fusion_uuid) & Consensus_scores < 1000]  %>% 
                                                                                        data.table::as.data.table %>% nrow,
          mhc_binders_class_I = dt[MHC %like% Class_i & Consensus_scores < 5000] %>% nrow,
          mhc_binders_class_II = dt[MHC %like% Class_ii & Consensus_scores < 5000] %>% nrow,
